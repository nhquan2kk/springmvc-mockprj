		...
		<div class="container-fluid page-body-wrapper">
			...
			<div class="main-panel">
				<div class="content-wrapper">
					<div class="row">
						<a class="my-4 text-black" href="${pageContext.request.contextPath}/screeningrooms/create">Create
							new screening room</a>
								...
								<div class="card-body">
									<div class="table-responsive pt-3">
										<table class="table table-bordered">
											...
											<tbody>
												 <c:if test="${not empty screeningrooms}">
												    <c:forEach items="${screeningrooms}" var="screeningroom" varStatus="index">
												        <tr>
												            <td><a href="#" data-toggle="modal" data-target="#updateModal-${screeningroom.id}">${screeningroom.room_name}</a></td>
												            <td>${screeningroom.totalSeat}</td>
												            <td>
												            	<form id="deleteForm" method="post"
																action="${pageContext.request.contextPath}/screeningrooms/delete">
																<input type="hidden" name="id"
																	value="${screeningroom.id }" />
																<button type="button" class="btn " data-toggle="modal"
																	data-target="#exampleModal-${screeningroom.id }">
																	<i style="font-size: 30px;"
																		class="mdi mdi-delete-forever btn-icon "></i>
																</button>
																<div class="modal fade"
																	id="exampleModal-${screeningroom.id }" tabindex="-1"
												            ...
												            <!-- Update Modal -->
												            <div class="modal fade" id="updateModal-${screeningroom.id}" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
												                <div class="modal-dialog" role="document">
												                    <div class="modal-content">
												                        <div class="modal-header">
												                            <h5 class="modal-title" id="updateModalLabel">Update Screening Room</h5>
												                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
												                                <span aria-hidden="true">&times;</span>
												                            </button>
												                        </div>
												                        <form action="${pageContext.request.contextPath}/screeningrooms/update" method="post">
												                            <div class="modal-body">
												                                <input type="hidden" name="id" value="${screeningroom.id}">
												                                <div class="form-group">
												                                    <label for="room_name">Name</label>
												                                    <input type="text" class="form-control" name="room_name" value="${screeningroom.room_name}" required>
												                                </div>
